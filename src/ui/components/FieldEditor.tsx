...
function TcpOptionsInput({ value, onCommit, rawFallback }: { value: Uint8Array; onCommit: (value: Uint8Array) => void; rawFallback: React.ReactNode }) {
  ...
  const number = (label: string, current: number | undefined, max: number, change: (value: number | undefined) => TcpOptions) => (
    <label className="flex items-center gap-1 text-[11px] text-zinc-400">
      <input type="checkbox" className="size-4 accent-cyan-500" checked={current !== undefined} onChange={(event) => update(change(event.target.checked ? 0 : undefined))} />
      {label}
      {current !== undefined && <input aria-label={`${label} value`} type="number" min={0} max={max} className="w-20 rounded border border-zinc-700 bg-zinc-950 px-1 py-0.5 font-mono text-zinc-200" value={current} onChange={(event) => update(change(Math.min(max, Math.max(0, Number(event.target.value)))))} />}
    </label>
  );
  return (
    <div className="flex flex-wrap gap-x-3 gap-y-1" role="group" aria-label="TCP options">
      ...
      <label className="flex items-center gap-1 text-[11px] text-zinc-400"><input type="checkbox" className="size-4 accent-cyan-500" checked={options.sackPermitted === true} onChange={(event) => update({ ...options, sackPermitted: event.target.checked || undefined })} />SACK permitted</label>
