...
export default function DiagramExportDialog({ packet, registry, onClose }: { packet: SerializedPacket; registry: Registry; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-2 sm:p-4" onClick={onClose}>
    <div ref={ref} role="dialog" aria-modal="true" aria-labelledby="diagram-export-title" className="flex h-[min(52rem,calc(100dvh-1rem))] w-[min(72rem,calc(100vw-1rem))] min-w-0 resize-y flex-col overflow-hidden rounded-xl border border-zinc-700 bg-zinc-900 shadow-2xl sm:resize" onClick={(event) => event.stopPropagation()}>
      <header className="flex items-center border-b border-zinc-800 px-5 py-3"><h2 id="diagram-export-title" className="text-[14px] font-semibold text-zinc-100">Export diagram</h2><button className="ml-auto cursor-pointer rounded p-1.5 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200" aria-label="Close diagram export dialog" onClick={onClose}><X className="size-4" /></button></header>
      <div className="flex min-h-0 flex-1 flex-col gap-4 px-5 py-4">
        <div className="grid shrink-0 gap-4 sm:grid-cols-3">
        <label className="text-[12px] text-zinc-400">Content<select className="mt-1 block w-full rounded border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-zinc-200 outline-none focus:border-cyan-600" value={scope} onChange={(event) => setScope(event.target.value)}><option value="all">Whole stack</option>{packet.layers.map((layer) => <option key={layer.uid} value={layer.uid}>{registry.get(layer.protocolId)?.name ?? layer.protocolId}</option>)}</select></label>
        <label className="text-[12px] text-zinc-400">Theme<select className="mt-1 block w-full rounded border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-zinc-200 outline-none focus:border-cyan-600" value={theme} onChange={(event) => setTheme(event.target.value as DiagramTheme)}><option value="print">Print (black on white)</option><option value="light">Light</option><option value="dark">Dark</option></select></label>
        <fieldset><legend className="mb-1 text-[12px] text-zinc-400">Format</legend><div className="flex gap-4 text-[13px] text-zinc-200">{(['svg', 'png'] as const).map((value) => <label key={value} className="flex items-center gap-1.5"><input type="radio" className="accent-cyan-500" checked={format === value} onChange={() => setFormat(value)} />{value.toUpperCase()}{value === 'png' ? ' (2×)' : ''}</label>)}</div></fieldset>
        </div>
        <div
      ...
      <footer className="flex flex-wrap items-center justify-end gap-2 border-t border-zinc-800 px-5 py-3">
        ...
        <button className="cursor-pointer rounded-md px-3 py-1.5 text-[13px] text-zinc-400 hover:text-zinc-200" onClick={onClose}>Cancel</button>
        ...
        <button className="flex cursor-pointer items-center gap-1.5 rounded-md bg-cyan-700 px-3 py-1.5 text-[13px] font-medium text-white hover:bg-cyan-600" onClick={() => void download()}><Download className="size-3.5" />Download {format.toUpperCase()}</button>
      </footer>
    </div>
  </div>;
}
